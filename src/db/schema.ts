...
export const roundCoffees = pgTable(
  ...
  (table) => ({
    ...
    uniqueCoffeePerRound: uniqueIndex("round_coffees_round_coffee_idx").on(table.roundId, table.coffeeId),
  }),
)
...
export const orderItems = pgTable(
  "order_items",
  {
    ...
    roundCoffeeId: uuid("round_coffee_id").notNull().references(() => roundCoffees.id, { onDelete: "restrict" }),
...
export const coffeesRelations = relations(coffees, ({ one, many }) => ({
  supplier: one(suppliers, { fields: [coffees.supplierId], references: [suppliers.id] }),
  roundCoffees: many(roundCoffees),
}))

export const roundsRelations = relations(rounds, ({ one, many }) => ({
  supplier: one(suppliers, { fields: [rounds.supplierId], references: [suppliers.id] }),
  roundCoffees: many(roundCoffees),
  orders: many(orders),
}))

export const roundCoffeesRelations = relations(roundCoffees, ({ one, many }) => ({
  round: one(rounds, { fields: [roundCoffees.roundId], references: [rounds.id] }),
  coffee: one(coffees, { fields: [roundCoffees.coffeeId], references: [coffees.id] }),
...
export const orderItemsRelations = relations(orderItems, ({ one }) => ({
  ...
  roundCoffee: one(roundCoffees, { fields: [orderItems.roundCoffeeId], references: [roundCoffees.id] }),
}))
