...
function ChartTooltipContent({
  ...
  formatter?: (...args: Array<unknown>) => React.ReactNode
  color?: string
}) {
  ...
  const tooltipLabel = React.useMemo(() => {
    ...
    const [item] = payload
    const key = `${labelKey || (item as { dataKey?: string }).dataKey || (item as { name?: string }).name || "value"}`
    const itemConfig = getPayloadConfigFromPayload(config, item, key)
    const labelConfig =
      typeof label === "string"
        ? (config[label] as (typeof config)[string] | undefined)
        : undefined
    const value =
    ...
    >
      ...
      <div className="grid gap-1.5">
        {payload.map((item, index) => {
          const key = `${nameKey || (item as { name?: string }).name || (item as { dataKey?: string }).dataKey || "value"}`
          const itemConfig = getPayloadConfigFromPayload(config, item, key)
                  ...
                  >
                    <div className="grid gap-1.5">
                      ...
                      <span className="text-muted-foreground">
                        {itemConfig?.label ||
                          (item as { name?: React.ReactNode }).name}
                      </span>
                    </div>
                    {(item as { value?: number }).value !== undefined && (
  ...
  return (
    ...
    >
      {payload.map((item) => {
        ...
        return (
          <div
            key={String((item as { value?: unknown }).value)}
            className={cn(
