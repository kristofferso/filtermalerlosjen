...
import { coffees, orderItems, orders, roundCoffees, rounds, suppliers } from "@/db/schema"
...
async function getOpenRoundRecord() {
  const [round] = await db.select().from(rounds).where(eq(rounds.status, "open")).limit(1)
  return round ?? null
}
...
export const getCustomerHomeData = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const [supplier] = await db.select().from(suppliers).where(eq(suppliers.id, openRound.supplierId)).limit(1)
...
export const getAdminDashboard = createServerFn({ method: "GET" }).handler(async () => {
  ...
  const roundIds = [openRound?.id, ...closedRoundRows.map((round) => round.id)].filter(Boolean) as Array<string>
  const [roundCoffeeRows, orderRows] = await Promise.all([
    roundIds.length > 0
      ? db.select().from(roundCoffees).where(inArray(roundCoffees.roundId, roundIds)).orderBy(roundCoffees.createdAt)
      : [],
    getOrderSummaries(roundIds),
  ])

  const suppliersById = new Map(supplierRows.map((supplier) => [supplier.id, supplier]))
  const roundCoffeesByRoundId = groupBy(roundCoffeeRows, (coffee) => coffee.roundId)
...
export const openRound = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    await assertAdmin()
    if (await getOpenRoundRecord()) throw new Error("A coffee round is already open")
    ...
    const [round] = await db
      .insert(rounds)
      .values({ supplierId: data.supplierId, status: "open", openedAt: new Date() })
      .returning()
...
export const closeRound = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [round] = await db
      .update(rounds)
      .set({ status: "closed", closedAt: new Date(), shippingKr: data.shippingKr, updatedAt: new Date() })
...
export const submitOrder = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const roundCoffeeRows = await db
      .select()
      .from(roundCoffees)
      .where(and(eq(roundCoffees.roundId, data.roundId), inArray(roundCoffees.id, items.map((item) => item.roundCoffeeId))))

    if (roundCoffeeRows.length !== items.length) throw new Error("Invalid coffee selection")

    const [order] = await db.insert(orders).values({ roundId: data.roundId, customerName: data.customerName }).returning()
...
export const updateOrderFlags = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    const [order] = await db
      .update(orders)
      .set({ paid: data.paid, collected: data.collected, updatedAt: new Date() })
